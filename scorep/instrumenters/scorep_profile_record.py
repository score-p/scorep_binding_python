...
class ScorepProfileRecord:
    """Similar to ScorepProfile but disables recording, when instrumenter is disabled.
    ...
    def __init__(self, scorep_bindings, enable_instrumenter=True):
        ...
        if not self.enable_instrumenter:
            self.scorep_bindings.disable_recording()

    def __del__(self):
        if not self.scorep_bindings.recording_enabled():
    ...
    def unregister(self):
        _unsetprofile()
        self.tracer_registered = False
        if self.scorep_bindings.recording_enabled():
            self.scorep_bindings.disable_recording()
    ...
    def region_begin(self, module_name, function_name, file_name, line_number):
        recording_enabled = self.scorep_bindings.recording_enabled()
